...
// 用于保护 addedURLs
func GetEnInfoDnsrepo(response string, DomainsIP *outputfile.DomainsIP) (*Utils.EnInfos, map[string]*outputfile.ENSMap) {
...
func Dnsrepo(domain string, options *Utils.LongOptions, DomainsIP *outputfile.DomainsIP) string {
	...
	result = result + "]}"
	res, ensOutMap := GetEnInfoDnsrepo(result, DomainsIP)
