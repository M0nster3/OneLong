...
// 用于保护 addedURLs
func GetEnInfoRacent(response string, DomainsIP *outputfile.DomainsIP) (*Utils.EnInfos, map[string]*outputfile.ENSMap) {
	...
	Utils.DomainTableShow(keyword, data, "Racent")
...
func Racent(domain string, options *Utils.ENOptions, DomainsIP *outputfile.DomainsIP) string {
	...
	res, ensOutMap := GetEnInfoRacent(string(resp.Body()), DomainsIP)
